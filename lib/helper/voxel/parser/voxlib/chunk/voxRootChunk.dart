...
class VoxRootChunk extends VoxChunk {
  ...
  final HashMap<int, VoxTransformChunk> transformChunks =
      HashMap<int, VoxTransformChunk>();
    ...
    while (childrenStream.available() > 0) {
      ...
      if (chunk1 != null) {
        ...
        if (chunk1 is VoxSizeChunk) {
          VoxChunk? chunk2 =
              VoxChunk.readChunk(childrenStream, ChunkFactory.XYZI);
          if (chunk2 != null) root.appendChunk(chunk2);
        }
      }
    }
  ...
  void processShapeChunk(VoxShapeChunk shapeChunk, GridPoint3 pos) {
    for (int modelId in shapeChunk.model_ids) {
      ...
      if (model.getVoxels().isNotEmpty) {
        VoxModelInstance instance =
            VoxModelInstance(model, GridPoint3.fromPoints(pos));
