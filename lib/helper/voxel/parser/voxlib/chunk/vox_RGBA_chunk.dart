...
class VoxRGBAChunk extends VoxChunk {
  ...
  late final List<String> palette;

  VoxRGBAChunk([List<String>? palette]) : super(ChunkFactory.RGBA) {
  ...
  static VoxRGBAChunk read(InputStream stream) {
    ...
    for (int i = 0; i < 255; i++) {
      ...
      int r = StreamUtils.readByte(stream);
      int g = StreamUtils.readByte(stream);
      int b = StreamUtils.readByte(stream);
      int a = StreamUtils.readByte(stream);
      chunk.palette[i + 1] = a.toRadixString(16).padLeft(2, "0") +
          (r).toRadixString(16).padLeft(2, "0") +
          (g).toRadixString(16).padLeft(2, "0") +
          b
              .toRadixString(16)
              .padLeft(2, "0"); //ARGBToABGR(abgr); //ABGRToARGB(abgr);
    }
    return chunk;
  }
